...
// below is top 10 common symtoms
...
// below is append to active array 

function cbox0() {
  var label = document.querySelector("label[for=C0").textContent;
  $.getJSON('/appendpb', {a: label}, function(data) {
  //     var html = '';
  //     $.each(data, function(index, item) {
  //         html += '<p>' + item.name + '</p>';
  //       });
  ...
  $.getJSON('/main', function(data) {
    ...
    $.each(data, function(index, item) {
        ...
        if (item.name == "terminate"){
          ...
          var txt = '<h4><strong>You probably have</strong></h4>';
          $.each(data,function(index,item){
